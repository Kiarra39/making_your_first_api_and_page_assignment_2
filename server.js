...
app.get('/status-info', (req,res)=>{
  ...
  if (code == 200) {
    ...
    let obj = {
        status: 503,
        message: "Service Unavailable: The server is not ready to handle the request. Common causes are a server that is down for maintenance or overloaded.",
    };
    return res.send(obj);
} else if (code == 504) {
    let obj = {
    ...
    let obj = {
        status: code,
        message: "Unknown Status Code: The provided status code is not recognized.",
    };
    return res.send(obj);
}
